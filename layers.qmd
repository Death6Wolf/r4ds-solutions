...
5.  In the first pair of plots, we see that setting `group = 1` results in the marginal proportions of `cut`s being plotted.
    In the second pair of plots, setting `group = color` results in the proportions of `color`s within each `color` being plotted. A more informative alternative is to use after_stat(count / sum(count)), which displays the proportion of each colour and cut combination relative to the entire dataset.
